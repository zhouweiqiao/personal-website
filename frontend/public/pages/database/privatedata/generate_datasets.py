...
def main():
    ...
    high_entropy_alloys.to_excel(f'datasets/high_entropy_alloys_{timestamp}.xlsx', index=False)
    metal_oxides.to_excel(f'datasets/metal_oxides_{timestamp}.xlsx', index=False)
    semiconductors.to_excel(f'datasets/semiconductors_{timestamp}.xlsx', index=False)
